...
if __name__ == "__main__":
    ...
    parser.add_argument('--collect_interval', type=int, default=None,
                        help='Collect self-replicator/fertile genomes (for end-of-run '
                             ...
                             'collection host-transfer dominates runtime but does NOT affect '
                             'the simulation trajectory. Defaults to --log_interval.')
    ...
    try:
        pop, stats = model.run_simulation(
            ...
            # Genome collection runs in Python (no in-graph jax.debug.callback,
            # which would force a ~90 ms/cycle host-sync barrier). Every-cycle
            # collection still costs ~85 ms/cycle (standing-set transfer + dict
            # scan); every log_interval cycles it is ~31 ms and archives the same
            # established genomes. Default the cadence to log_interval.
            collect_interval=(
